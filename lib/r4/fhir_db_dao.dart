...
class FhirDbDao {
  ...
  Future<Resource> save(
    Resource resource, [HiveCipher? cipher]
  ) async {
  ...
  Future<bool> saveAll(
    List<Resource> resources, [HiveCipher? cipher]
  ) async {
    for (final Resource resource in resources) {
      try {
        await save(resource, cipher);
      } catch (e) {
        return false;
      }
    }
  ...
  Future<bool> addAll(List<Resource> resources, [HiveCipher? cipher]) async =>
      saveAll(resources, cipher);
  ...
  Future<List<Resource>> find(
    ...
    if (resource != null &&
      ...
      return _search(resourceType: resourceType, finder: finder, cipher: cipher);
    } else {
