...
class FhirDb {
  ...
  Set<Dstu2ResourceType> _types = <Dstu2ResourceType>{};
  ...
  Future<Box<Map<dynamic, dynamic>>> _getBox(
      {required Dstu2ResourceType resourceType, HiveCipher? cipher}) async {
    ...
    try {
      if (_types.contains(resourceType)) {
        ...
        await box.put(
            'types',
            _types
                .map((Dstu2ResourceType e) => Resource.resourceTypeToString(e))
                .toList());
        return true;
      }
    } catch (e) {
